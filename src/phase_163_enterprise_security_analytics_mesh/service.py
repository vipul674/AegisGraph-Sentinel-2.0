...
from .store import EnterpriseSecurityAnalyticsMeshStore


class EnterpriseSecurityAnalyticsMeshService:
    def __init__(self, store: EnterpriseSecurityAnalyticsMeshStore):
        self.store = store

    def create_record(self, tenant_id: str, record_id: str, name: str,
                       status: str = "ACTIVE", metadata: Dict[str, Any] = None) -> EnterpriseSecurityAnalyticsMeshRecord:
        record = EnterpriseSecurityAnalyticsMeshRecord(
            record_id=record_id,
            tenant_id=tenant_id,
            name=name,
            status=status,
        ...
        event = EnterpriseSecurityAnalyticsMeshEvent(
    ...
    def create_alert(self, tenant_id: str, alert_id: str, title: str,
                     severity: str) -> EnterpriseSecurityAnalyticsMeshAlert:
        alert = EnterpriseSecurityAnalyticsMeshAlert(
            alert_id=alert_id,
            tenant_id=tenant_id,
            title=title,
            severity=severity,
...
def get_service() -> EnterpriseSecurityAnalyticsMeshService:
    from .store import get_store
    return EnterpriseSecurityAnalyticsMeshService(get_store())
