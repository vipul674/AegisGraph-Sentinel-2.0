...
def test_create_record():
    ...
    assert resp.json()["record_id"] == "rec-integ-67-001"


def test_list_records():
    resp = client.get("/api/v1/phase67/records", headers=HEADERS)
    assert resp.status_code == 200
    data = resp.json()
...
def test_get_record():
    resp = client.get("/api/v1/phase67/records/rec-integ-67-001", headers=HEADERS)
    assert resp.status_code == 200
    assert resp.json()["record_id"] == "rec-integ-67-001"


def test_analytics():
    resp = client.get("/api/v1/phase67/analytics", headers=HEADERS)
    assert resp.status_code == 200
    data = resp.json()
