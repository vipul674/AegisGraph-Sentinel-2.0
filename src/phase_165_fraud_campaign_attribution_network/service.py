...
from .store import FraudCampaignAttributionNetworkStore


class FraudCampaignAttributionNetworkService:
    def __init__(self, store: FraudCampaignAttributionNetworkStore):
        self.store = store

    def create_record(self, tenant_id: str, record_id: str, name: str,
                       status: str = "ACTIVE", metadata: Dict[str, Any] = None) -> FraudCampaignAttributionNetworkRecord:
        record = FraudCampaignAttributionNetworkRecord(
            record_id=record_id,
            tenant_id=tenant_id,
            name=name,
            status=status,
        ...
        event = FraudCampaignAttributionNetworkEvent(
    ...
    def create_alert(self, tenant_id: str, alert_id: str, title: str,
                     severity: str) -> FraudCampaignAttributionNetworkAlert:
        alert = FraudCampaignAttributionNetworkAlert(
            alert_id=alert_id,
            tenant_id=tenant_id,
            title=title,
            severity=severity,
...
def get_service() -> FraudCampaignAttributionNetworkService:
    from .store import get_store
    return FraudCampaignAttributionNetworkService(get_store())
