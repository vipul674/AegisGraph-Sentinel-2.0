...
from .store import CyberThreatFusionPlatformStore


class CyberThreatFusionPlatformService:
    def __init__(self, store: CyberThreatFusionPlatformStore):
        self.store = store

    def create_record(self, tenant_id: str, record_id: str, name: str,
                       status: str = "ACTIVE", metadata: Dict[str, Any] = None) -> CyberThreatFusionPlatformRecord:
        record = CyberThreatFusionPlatformRecord(
            record_id=record_id,
            tenant_id=tenant_id,
            name=name,
            status=status,
        ...
        event = CyberThreatFusionPlatformEvent(
    ...
    def create_alert(self, tenant_id: str, alert_id: str, title: str,
                     severity: str) -> CyberThreatFusionPlatformAlert:
        alert = CyberThreatFusionPlatformAlert(
            alert_id=alert_id,
            tenant_id=tenant_id,
            title=title,
            severity=severity,
...
def get_service() -> CyberThreatFusionPlatformService:
    from .store import get_store
    return CyberThreatFusionPlatformService(get_store())
