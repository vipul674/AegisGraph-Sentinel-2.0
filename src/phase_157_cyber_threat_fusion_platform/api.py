...
from .schemas import CyberThreatFusionPlatformCreateSchema, CyberThreatFusionPlatformAlertSchema
from .store import get_store, CyberThreatFusionPlatformStore
from .service import CyberThreatFusionPlatformService
from .analytics import CyberThreatFusionPlatformAnalytics

router = APIRouter(prefix="/api/v1/phase157", tags=["Phase 157: Cyber Threat Fusion Platform"])
...
def get_svc(store: CyberThreatFusionPlatformStore = Depends(get_store)) -> CyberThreatFusionPlatformService:
    return CyberThreatFusionPlatformService(store)


@router.post("/records")
def create_record(
    payload: CyberThreatFusionPlatformCreateSchema,
    tenant_id: str = Depends(verify_auth),
    svc: CyberThreatFusionPlatformService = Depends(get_svc)
):
...
@router.post("/alerts")
def create_alert(
    payload: CyberThreatFusionPlatformAlertSchema,
    tenant_id: str = Depends(verify_auth),
    svc: CyberThreatFusionPlatformService = Depends(get_svc)
):
...
def get_analytics(
    tenant_id: str = Depends(verify_auth),
    store: CyberThreatFusionPlatformStore = Depends(get_store)
):
